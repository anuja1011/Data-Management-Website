...
<body>
	...
	<div class="content">
		<h3 align="center">Click the below Button to Refresh the Giving Balance of All the users</h3>
		
		<form name="Refreshform" action="Controller" method="post" id="form">
		<input type="hidden" name="action" value="RefreshBalance">
		
		<input type="submit" class="button_giving" value="Refresh">
		</form>
		...
		<table class="table">
			...
			<%ArrayList<GivingBalancePOJO> arr_rep = (ArrayList<GivingBalancePOJO>)request.getAttribute("refresh"); %>
			...
			<tr>
				...
				<td>
				<%=arr_rep.get(x).getAmount() %>
				</td>
				<td>
				<%=arr_rep.get(x).getTransactionId() %>
				</td>
			</tr>
			<%} %>
		</table>
